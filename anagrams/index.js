...
function anagrams(stringA, stringB) {
  ...
  const stringAValue = stringA.split("").sort().join("");
  const stringBValue = stringB.split("").sort().join("");
 return stringAValue === stringBValue 
...
/** 
  ...
  for(let i =0; i<stringA.length; i++){
    let char = stringA[i];
    stringACharCount[char] = stringACharCount[char] + 1 || 1
  }
...
console.log(anagrams('Kalisa','salika'));
