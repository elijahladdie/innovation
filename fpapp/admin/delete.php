...
if(!empty($_GET['id']) &&  !empty($_GET['class'])){
 ...
 if(isset($_POST['btn'])){
	...
	$update = mysqli_query($conn,"INSERT  INTO student (First_name,Last_name,phone,phone1,class,payment)VALUES('$fname','$lname','$phone','$phone1','$tbl','$payment')");
	$update = mysqli_query($conn,"DELETE from inactivestudent where class='$tbl' and No='$id' ");
...
$select= mysqli_query($conn,"SELECT *from inactivestudent  where class='$tbl' AND No=$id");
		...
		<tr>
			...
			<td>
                <select name="describe" id="" class="form-select">
                        <option value="Not important">Mention Reason</option>
